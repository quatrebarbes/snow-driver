...
<limite>Un modèle déclaré manuellement par l'application hôte sans liste de champs modifiables ni conversion (par exemple avec $guarded vide) n'expose aucun champ au travers de son code : un outil hôte restreignant les champs affichés à ceux déclarés par le modèle n'en montrera alors que les champs techniques. Le remède relève de l'application hôte (déclarer ses champs modifiables ou ses conversions) et non du driver.</limite>
