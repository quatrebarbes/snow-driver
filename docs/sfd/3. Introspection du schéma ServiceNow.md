...
<exigence id="EX-331">Un champ dont le type ServiceNow n'admet aucun équivalent de conversion Eloquent (EX-327) — chaîne courte ou texte long — doit néanmoins être déclaré en conversion explicite vers 'string' dans le modèle généré, par souci de lisibilité : le type de chaque champ reste visible depuis le code du modèle, y compris pour les champs qui n'appellent aucune transformation de valeur.</exigence>
